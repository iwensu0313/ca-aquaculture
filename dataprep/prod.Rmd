...
* For states with at least 1 non-missing value, use state average
* For states with all missing values, use the regionalaverage.
...
# Check if every state has at least 1 value
NA_count <- fishdolla %>% 
  group_by(State) %>% 
...
# Combine State Region Information
...
fish_gf <- fishdolla %>% 
  ...
  mutate(Gapfill = ifelse(is.na(Value), 1, 0),
         GF_Method = ifelse(Gapfill == 1 & nonNAs > 0, "State Average", NA),
         GF_Method = ifelse(Gapfill == 1 & nonNAs == 0, "Region Average", GF_Method))
...
fish_US_map <- fish_sales %>% 
  rename(map_data = Total)

write.csv(fish_US_map, file.path(intdata, "fish_us_map.csv"))
