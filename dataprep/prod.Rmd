...
sp_fix <- secsplit %>% 
  mutate(
    Product_Type = ifelse(is.na(Product_Type), "TEMP", Product_Type), 
...
# remove species names and TEMP from Product_Type
sp_remove <- str_c(c(sp_names, "TEMP", "(EXCL GRASS)"), collapse="|")
totalfish <- sp_fix %>% 
  mutate(Product_Type = ifelse(str_detect(Product_Type, sp_remove), NA, Product_Type))
...
test <- totalfish %>% 
  ...
  select(-Wholesale_Type, -Region_Code, -Commodity)
...
Within totals, just select raw,  not calculated, data. Select for most recent year (2013). Remove wholesale and retail information - the units for this is PCT BY OUTLET.
...
dolperop <- totalfish %>% 
  ...
  select(Year, Region, Species, Unit, Value) %>% 
...
rawfish <- totalfish %>% 
  filter(Product_Type != "WHOLESALE" & Product_Type != "RETAIL",
         Unit %in% c("OPERATIONS", "HEAD", "LB", "DOLLARS", "EGGS"),
         Region != "US") %>%
  select(-Wholesale_Type, -Region_Code) %>% 
  filter(Year == 2013) %>% 
...
write.csv(NA_count, file.path(intdata, "States_Totals_2013/data_type_NAs.csv"))

```

### Gapfill

Yipes.. lots of gapfilling using linear regression.. lots of missing data!!

```{r}

fishdolla <- rawfish %>% filter(Unit == "DOLLARS")
## Compare models to select a gapfilling method
mod1 <- lm(Value ~ r2, data = rawfish, na.action="na.exclude")
mod2 <- lm(score ~ r2 + wgi_score, data = rawfish, na.action="na.exclude")
